...
void sys_log_libc(const char *message) { kprintf("%s", message); }
...
off_t sys_seek(int fd, off_t offset, int whence) {
  ...
  if (!gp_current_process->fd_table[fd]) {
    kprintf("[SYS_SEEK] Invalid fd\n");
    return -1; /* invalid fd */
  }
  ...
  case SEEK_END:
    gp_current_process->fd_table[fd]->position =
        gp_current_process->fd_table[fd]->size;
    gp_current_process->fd_table[fd]->position += offset;
    return gp_current_process->fd_table[fd]->position;
  default:
    kprintf("[SYS_SEEK] Whence is neither SEEK_SET, SEEK_CUR, or SEEK_END.\n");
    return -1;
  }
}
...
void *sys_vm_map(void *addr, size_t size, int prot, int flags, int fd,
  ...
  if (!(flags & MAP_ANON)) {
    ...
    __builtin_memset((PAGING_VIRTUAL_OFFSET + phys_base), 0, pages * PAGE_SIZE);
    if (phys_base == NULL) {
      // out of memory
      for (;;)
...
void syscall_dispatcher(Registers regs) {
  ...
  case SYS_SEEK: {
    ...
    register int fd asm("r8") = fd;
    register off_t offset asm("r9") = offset;
    register int whence asm("r10") = whence;

    off_t ret = sys_seek(fd, offset, whence);
    if (ret != -1) {
      regs.r15 = ret;
    } else {
      kprintf("Sys seek errors\n");
      for (;;)
        ;
    }
