...
struct boundary_tag* l_kfreePages[MAXEXP];		//< Allowing for 2^MAXEXP blocks
...
static inline int getexp( unsigned int size )
{
	...
	{
		#ifdef DEBUG
		kprintf("getexp returns -1 for %i less than MINEXP\n", size );
		#endif
	...
	#ifdef DEBUG
	kprintf("getexp returns %i (%i bytes) for %i size\n", shift - 1, (1<<(shift -1)), size );
	#endif
...
static void dump_array()
{
	...
	kprintf("System memory allocated: %i\n", l_allocated );
	kprintf("Memory in used (kmalloc'ed): %i\n", l_inuse );
...
static inline void insert_tag( struct boundary_tag *tag, int index )
{
	...
	if ( l_kfreePages[ realIndex ] != NULL ) 
	{
		l_kfreePages[ realIndex ]->prev = tag;
		tag->next = l_kfreePages[ realIndex ];
	}

	l_kfreePages[ realIndex ] = tag;
}

static inline void remove_tag( struct boundary_tag *tag )
{
	if ( l_kfreePages[ tag->index ] == tag ) l_kfreePages[ tag->index ] = tag->next;
...
static inline struct boundary_tag* absorb_right( struct boundary_tag *tag )
{
	struct boundary_tag *right = tag->split_right;
		...
		remove_tag( right );		// Remove right from kfree pages.
...
static struct boundary_tag* allocate_new_tag( unsigned int size )
{
	...
	struct boundary_tag *tag;
		...
		#ifdef DEBUG
		kprintf("Resource allocated %x of %i pages (%i bytes) for %i size.\n", tag, pages, pages * l_pageSize, size );

		l_allocated += pages * l_pageSize;

		kprintf("Total memory usage = %i KB\n",  (int)((l_allocated / (1024))) );
		#endif
...
void *kmalloc(size_t size)
{
	...
	liballoc_lock();
		...
		// Find one big enough.
			tag = l_kfreePages[ index ];				// Start at the front of the list.
			while ( tag != NULL )
			{
		...
		// We have a kfree page.  Remove it from the kfree pages list.
		...
		if ( ((int)(remainder) > 0) /*&& ( (tag->real_size - remainder) >= (1<<MINEXP))*/ )
		{
			...
			{
				#ifdef DEBUG
				kprintf("Seems to be splittable: %i >= 2^%i .. %i\n", remainder, childIndex, (1<<childIndex) );
				#endif
				...
				#ifdef DEBUG
				kprintf("Old tag has become %i bytes, new tag is now %i bytes (%i exp)\n", tag->real_size, new_tag->real_size, new_tag->index );
				#endif
			}	
		}
...
void kfree(void *ptr)
{
	...
	liballoc_lock();
		...
		{	
			...
			if ( l_completePages[ index ] == MAXCOMPLETE )
			{
				...
				#ifdef DEBUG
				l_allocated -= pages * l_pageSize;
				kprintf("Resource kfreeing %x of %i pages\n", tag, pages );
				dump_array();
				#endif
	...
	#ifdef DEBUG
	kprintf("Returning tag with %i bytes (requested %i bytes), which has exponent: %i\n", tag->real_size, tag->size, index ); 
	dump_array();
	#endif
