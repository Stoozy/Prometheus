...
void kernel_main(multiboot_info_t* mbd, unsigned int magic) {
	...
	//mmap_entry* entry = mbd->mmap_addr;
	//while(entry < (mbd->mmap_addr + mbd->mmap_length)) {
	//	// do something with the entry
	//	entry = (mmap_entry*) ((unsigned int) entry + entry->size + sizeof(entry->size));
	//	if(entry->type == MULTIBOOT_MEMORY_AVAILABLE){
	//		printf("Available Memory entry at %llx of size %u of length: %lld\n",  entry->addr, entry->size, entry->len);
	//	}
	//}
